...
export async function signup(lng: string, formData: FormData) {
  ...
  if (password !== repeatPassword) {
    return redirect(`/${lng}/register?error=passwords_dont_match`)
  }

  // Validate password length
  if (password.length < MIN_CHARACTERS_NUMBER) {
    return redirect(`/${lng}/register?error=password_too_short&min=${MIN_CHARACTERS_NUMBER}`)
  }
  ...
  if (error) {
    return redirect(`/${lng}/register?error=signup_failed`)
  }
  ...
  if (userData?.user?.id) {
    ...
    redirect(pendingId ? `/${lng}/login?pendingId=${pendingId}` : `/${lng}/error`)
  }
}
